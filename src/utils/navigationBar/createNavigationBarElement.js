...
import { NavigationBarItem } from '../../components'
import {
  TYPE_DIVIDER,
  TYPE_ITEM,
  TYPE_PARENT
} from '../../data/navigationBarElementTypes'
...
const createDividerElement = ({ itemProps }) => <Divider {...itemProps} />
...
const createParentElement = ({ itemProps, icon, text, items }) => (
  <Menu.Item {...itemProps}>
    ...
    <Menu.Menu>{items.map(createNavigationBarElement)}</Menu.Menu>
  </Menu.Item>
)

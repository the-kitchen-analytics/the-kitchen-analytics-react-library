...
const buildStatisticsData = (rawData) => {
  ...
  const generalDataStatistics = Object.freeze([
...

    {
      ...
      renderValue: () => <Price euro>{totalWorkerIncome}</Price>
    }
  ])
  ...
  if (daysCount > 1) {
    const averageStatistics = Object.freeze([
...

      {
        ...
        renderValue: () => <Price euro>{totalWorkerIncome / daysCount}</Price>
      },

      {
        ...
        renderValue: () => (
          <Price euro>{totalWorkerIncome / operationsCount}</Price>
        )
      }
    ])
